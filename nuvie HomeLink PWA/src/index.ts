...
const deviceList = (room: null | string) => { //Get device list and show them in DOM
    $('#show-devices').html("");
...
const init = () => {
    ...
    const show_rooms = () => {
        ...
        for (let name in data.rooms) {
            ...
            if (track != 2) { (track = track + 1) } else { break };
        }
...
const eventListnerReset = () => {
    ...
    $('.show_roomlist').off().on('click', (e) => {
        ...
        setTimeout(()=> {
            ...
            if (found == false) $('.active_room').removeClass('active_room');
        }, 500)
...
$('.nav_navigator').on('click', (e) => {
    ...
    const actionType = (dom: 'acc'| 'metrics', action: 0|1) => {
        if (dom == 'acc')
